...
- [ ] **Step 1: Read `e2e-smoke.test.ts` fully** — copy its fixture architecture verbatim (hoisted temp dir, real `initializeDatabase()`, real temp audio file, boundary mocks: electron / config / file-storage / vector-store; it drives `processDownload` then `transcribeManually` with `autoTranscribe: false`). Its config mock (:103-128) carries the `summarization` block (P3) but NOT `openaiApiKey`/`whisperModel` — don't hunt for them there; Step 2 writes a fresh complete mock for this file.
