...
- [ ] **Step 1: Failing tests:**
  1. `parkQueueItem(id, parkedUntilIso)`: after it, the row has `status='pending'`, `parked_until=parkedUntilIso`, `first_parked_at` set (to ~now) — and **`retry_count` UNCHANGED** (seed a row with retry_count=2 via the generic transitions first, then park, then assert still 2). A second park updates `parked_until` but **keeps the original `first_parked_at`** (COALESCE).
  2. `getRunnableQueueItems()`: returns pending rows whose `parked_until` is NULL or `<= now`; a row parked into the future is EXCLUDED; same ordering as `getQueueItems` (`retry_count ASC, created_at ASC`). A row parked in the past IS returned (park expiry = runnable again, no extra transition needed).
  3. `clearParking(id)`: nulls both columns (used on successful stage completion).
...
export function parkQueueItem(id: string, parkedUntilIso: string): void {
  run(
    `UPDATE transcription_queue
     SET status = 'pending', parked_until = ?, first_parked_at = COALESCE(first_parked_at, CURRENT_TIMESTAMP), progress = 0
     WHERE id = ?`,
    [parkedUntilIso, id]
  )
}
 ...
 *  Everything else keeps using getQueueItems unchanged. */
export function getRunnableQueueItems(): (QueueItem & { filename?: string })[] {
  return queryAll<QueueItem & { filename?: string }>(`
    ...
    WHERE tq.status = 'pending' AND (tq.parked_until IS NULL OR tq.parked_until <= datetime('now'))
...
- [ ] **Step 1: Failing tests** (house fixture; drive via `processQueueManually` with mocked providers — patch the provider factories or the Gemini mock to throw the typed errors):
  ...
  6. Successful completion clears parking columns (park into past → succeed → both NULL).
...
```ts
        ...
        if (error instanceof ProviderRateLimitError) {
          ...
          // retry budget — park without burning retry_count.
          const firstParked = queryOneFirstParkedAt(item.id) // small SELECT helper or inline queryOne
          const parkedTooLong = firstParked && Date.now() - new Date(firstParked).getTime() > 24 * 60 * 60 * 1000
          if (parkedTooLong) {
            ...
            const parkedUntil = new Date(Date.now() + delayMs).toISOString()
            parkQueueItem(item.id, parkedUntil)
            updateRecordingTranscriptionStatus(item.recording_id, 'pending')
            console.log(`[Transcription] Parked ${item.id} until ${parkedUntil} (${error.provider} 429)`)
          }
          continue
        }
        // ... existing generic failure path unchanged ...
```
  (`queryOneFirstParkedAt`: read the row's current `first_parked_at` via `queryOne` from database — export a tiny `getQueueItemById(id)` if cleaner; pick one and keep it in database.ts.)
  (d) Success path: right after `updateQueueItem(item.id, 'completed')` (~:201) add `clearParking(item.id)`.
  (e) NON_RETRYABLE additions: `'quota still exhausted after 24h',` and `'quota exhausted',` (covers OpenAI `insufficient_quota` mapping — ALSO map it: in `whisper-asr.ts`'s non-OK branch, when the response body contains `insufficient_quota`, throw `Error('OpenAI quota exhausted — check billing, then Retry all')` (§7.1); add that mapping + 1 test in whisper-asr.test.ts as part of this task).
...
- [ ] **Step 2: AC5 evidence:** the per-stage check (P2) + key-fix re-pend (Task 3) + Stage-2 resume (P1) chain — name the tests proving: mid-queue key removal → items terminal-fail with the §7.1 message AFTER persisting full_text; key save re-pends them; they complete WITHOUT re-running ASR (Stage-2-only resume); chip showed the count (Task 4 test).
