...
- **Stage 1 — ASR:** file-existence check (today's `:368-375`) becomes **Stage-1-only**. `getAsrProvider().transcribe()` → upsert the `transcripts` row with `full_text`, `language`, `word_count`, `transcription_provider` (`'gemini'` | `'openai-whisper'`), `transcription_model`; Stage-2 fields untouched. The upsert must **never overwrite an existing non-NULL `summary`/`summarization_provider`** (replaces today's blind INSERT OR REPLACE for this path).
- **Stage 2 — Analysis:** `getLlmProvider().generate()` (analysis + meeting-selection, then validator §5.2) → one UPDATE: `summary`, `action_items`, `topics`, `key_points`, `title_suggestion`, `question_suggestions`, `summarization_provider`, `summarization_model`. **JSON-extraction failure THROWS** (this intentionally changes today's swallow-and-complete behavior at `:531-534`): the queue retries Stage 2; after `MAX_RETRY_ATTEMPTS` the item is terminal-failed, the marker stays NULL, and any pre-existing summary is untouched. No sentinel strings are ever written. §7's table reflects this — the rev 1 contradiction (sentinel + retryable) is resolved in favor of throw-and-retry.
...
- **Stage-2 re-run tail semantics:** actionables are **delete-and-replace** for the recording (today's append duplicates them, `:615-640`); auto-rename runs only on first-ever Stage 2 completion for the recording (never on re-runs); meeting re-selection may re-link (documented behavior). RAG re-indexing is already idempotent by recordingId (`vector-store.ts:192-201`).
...
- Progress stages preserved: `reading_file 5 → transcribing 20 → analyzing 50 → detecting_actionables 75 → indexing 85 → complete 100`. `transcription_status` stays `'processing'` through Stage 2 (§5.6 defines the failure-state UI).
- Missing-key preflight (`transcription.ts:108-131`) generalizes: validate the key for the **selected** ASR provider and the **selected** LLM provider; failure messages use the exact §7.1 strings.
...
- **New IPC `download-service:ensure-baseline`** `(deviceSerial: string, filenames: string[]) → { created: boolean }`: if no rows exist for the serial, snapshot all current filenames and return `created: true`. Explicit call — `getFilesToSync` stays a pure read (no side-effecting getter).
...
- **Auto-sync flow change** (`useDeviceSubscriptions.ts:73-150` and `:189-267`): read the serial from the device service state (`hidock-device.ts:38` — `serialNumber: string | null`); call `ensure-baseline` first; if it returns `created: true`, skip this sync cycle (log via QA pattern); else call `getFilesToSync` with `{ auto: true, deviceSerial }`. **`serialNumber === null` → fall back to manual semantics (no baseline logic) + QA log** — never key a baseline on null.
...
- **Library failure state:** on terminal Stage-2 failure, `transcription_status = 'error'` (existing badge), and the recording detail panel renders the existing `full_text` with an inline notice — "Summary failed — Re-summarize" — whose button calls `transcription:resummarize`. This is also the resummarize affordance (rev 1 shipped the IPC with no caller). During Stage 2 the badge remains `processing` (unchanged mapping).
- **Aggregate failure chip (§7.3):** the Library header shows "N transcriptions failed — Retry all" when N > 0 (the single non-silent surface; silent ≠ invisible failures).
- Auto toggles remain on the Device page (`Device.tsx:1015-1047`). QA logging obeys `qaLogsEnabled` patterns.

### 5.7 Queue hardening

- **`addToQueue` dedupe:** skip insert when a `pending`/`processing` item exists for the `recording_id` (guards the dual-trigger races: `download-service.ts:489-503` + `recording-watcher.ts:206-214`, and resummarize-while-queued).
...
### 5.8 DB migration recipe (explicit — protects the 7a3-stabilized first-launch path)

1. Add `summarization_provider TEXT`, `summarization_model TEXT` to the `transcripts` CREATE TABLE in SCHEMA **and** add a new `MIGRATIONS[v]` entry with try/catch-guarded `ALTER TABLE ADD COLUMN`s (v16 pattern, `database.ts:957-963`); bump `SCHEMA_VERSION`.
2. Same migration creates `sync_baseline_files` (CREATE TABLE IF NOT EXISTS — safe both paths).
3. **Backfill** (same migration): `UPDATE transcripts SET summarization_provider='gemini', summarization_model=transcription_model WHERE full_text IS NOT NULL AND summarization_provider IS NULL` — existing fused-flow transcripts are Stage-2-complete by definition; without this, every historical recording would look resumable.
4. Extend `e2e-smoke.test.ts` (the 7a2b18 guard) to assert the new columns exist on a fresh boot.
...
### 7.2 Quota parking (changes the retry machinery — scoped)

HTTP 429 does **not** increment `retry_count`. The item is parked: re-queued after `Retry-After` when present, else 30 min; parking capped at 24 h, after which it becomes a terminal failure. (Rationale: Ollama Cloud quota windows reset on 5-hour/weekly cycles; the existing 30/60/120 s backoff burns all retries in ~4 minutes against an hours-long window.)

### 7.3 Recovery loop

- **Key-fix re-pend:** when a provider key is saved in Settings, matching terminal failures re-pend: `UPDATE transcription_queue SET status='pending', retry_count=0 WHERE status='failed' AND error_message LIKE '%<provider marker>%'`.
- **Aggregate chip:** "N transcriptions failed — Retry all" in the Library header (§5.6); Retry-all performs the same re-pend for all failed items.
...
- **AC5:** with the Ollama key removed mid-queue, items terminal-fail with the §7.1 message; saving a valid key re-pends them automatically and they complete **without re-running ASR**; the Library chip shows the count until then.
- **AC6:** `transcription:resummarize` (via the detail-panel button) regenerates the summary with the currently selected LLM without touching `full_text`, works when local audio is deleted, keeps the old summary if the re-run fails, and does not duplicate actionables.
- **AC7:** existing Gemini-default configs are **behavior-identical** (defaults resolve to gemini/gemini, all existing tests green; the config file gaining new sections on next save is expected).
- **AC8:** queueing the same recording twice produces one transcription (dedupe + short-circuit); `recordings:transcribe` goes through the queue.
