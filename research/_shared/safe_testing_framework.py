...
class SafeCommandTester:
    ...
    def attempt_device_recovery(self) -> bool:
        ...
        logger.info(
            "SafeTester", "attempt_device_recovery", f"Attempting device recovery (attempt {self.recovery_count + 1})"
        )
    ...
    def safe_command_test(self, command_id: int, parameters: bytes, description: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            if response:
                ...
                logger.info(
                    "SafeTester", "safe_command_test", f"Command {command_id} SUCCESS: Response in {elapsed_time:.2f}s"
                )
            else:
                result["status"] = "no_response"
                logger.info(
                    "SafeTester", "safe_command_test", f"Command {command_id} NO_RESPONSE in {elapsed_time:.2f}s"
                )
            ...
            logger.warning("SafeTester", "safe_command_test", f"Command {command_id} ERROR: {e}")
            ...
            if not self.test_device_health():
                ...
                if self.recovery_count < self.max_recovery_attempts:
                    if self.attempt_device_recovery():
                        ...
                        logger.error("SafeTester", "safe_command_test", "CRITICAL: Device recovery failed")
                else:
                    result["device_healthy_after"] = False
                    logger.error(
                        "SafeTester",
    ...
    def _format_response(self, response) -> Dict[str, Any]:
        ...
        if isinstance(response, dict):
            ...
            return {"type": "raw", "data": str(response)}

    def run_parameter_test_batch(
        self, command_id: int, parameter_list: List[Tuple[bytes, str]]
    ) -> List[Dict[str, Any]]:
        """
