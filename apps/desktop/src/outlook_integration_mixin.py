...
from outlook_calendar_service import MeetingMetadata, OutlookCalendarService
...
class OutlookIntegrationMixin:
    ...
    def _initialize_outlook_integration(self):
        ...
        if not hasattr(self, "outlook_service"):
            ...
            if self.outlook_service.is_enabled():
                threading.Thread(target=self._auto_authenticate_outlook, daemon=True).start()
                ...
                for file_data in files_dict:
                    ...
                    if file_datetime:
                        ...
                        meeting = self.outlook_service.find_meeting_for_audio_file(
                            file_datetime, duration_seconds=file_data.get("duration", 0)
                        )
    ...
    def _create_meeting_fields(self, meeting: MeetingMetadata) -> Dict:
        ...
        organizer = (
            meeting.organizer.split("@")[0] if meeting.organizer and "@" in meeting.organizer else meeting.organizer
        )
    ...
    def get_meeting_details_for_file(self, filename: str) -> Optional[Dict]:
        ...
        file_data = next((f for f in self.displayed_files_details if f.get("name") == filename), None)
    ...
    def get_outlook_status(self) -> Dict[str, str]:
        ...
        if not hasattr(self, "outlook_service"):
            return {"available": "Not initialized", "enabled": "No", "authenticated": "No", "cache_age": "N/A"}
