...
class AsyncCalendarMixin:
    ...
    def _initialize_async_calendar_components(self):
        ...
        try:
            ...
            # Update display format for existing cached meetings to ensure consistency
            # This fixes the issue where cached meetings show different format than live refresh
            self._calendar_cache_manager.update_display_format_for_existing_cache()
            ...
            self._calendar_worker_thread = threading.Thread(
                target=self._calendar_worker_loop, daemon=True, name="CalendarWorker"
            )
    ...
    def _process_single_file_sync_request(self, work_item: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                "AsyncCalendar",
                "single_file_sync",
                f"Completed single file sync for {enhanced_file.get('name', 'unknown')}",
            )
    ...
    def _enhance_single_file_with_calendar_data(self, file_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Find meeting from calendar using config tolerance
            from config_and_logger import load_config

            config = load_config()
            tolerance_minutes = config.get("calendar_tolerance_minutes", 20)
            meeting = self._calendar_integration.find_meeting_for_recording(
                file_datetime, tolerance_minutes=tolerance_minutes
            )
    ...
    def _create_empty_meeting_fields(self) -> Dict[str, Any]:
        ...
        # During connection when loading cached files, NEVER show "Syncing..."
            ...
            hasattr(self, "_calendar_sync_status")
            and self._calendar_sync_status == "syncing"
            and hasattr(self, "_calendar_processed_files")
            and hasattr(self, "_calendar_total_files")
            and self._calendar_processed_files < self._calendar_total_files
        ):
    ...
    def _enhance_files_with_batch_calendar_data(self, files_dict: List[Dict]) -> List[Dict]:
        ...
        for chunk_start_date, chunk_files in date_chunks.items():
            try:
                ...
                progress = (
                    self._calendar_processed_files / max(1, self._calendar_total_files)
                    if self._calendar_total_files > 0
                    else 1.0
                )
                ...
                # Try individual file processing as fallback for failed chunks
                logger.info(
                    "AsyncCalendar",
                    "batch_enhance",
                    f"Attempting individual processing for {len(chunk_files)} files from failed chunk",
                )
                for file_data in chunk_files:
                    try:
                        ...
                        logger.debug(
                            "AsyncCalendar",
                            "batch_enhance",
                            f"Individual processing succeeded for {file_data.get('name', 'unknown')}",
                        )
                    except Exception as individual_error:
                        logger.debug(
                            "AsyncCalendar",
                            "batch_enhance",
                            f"Individual processing failed for {file_data.get('name', 'unknown')}: {individual_error}",
                        )
                        # Only if individual processing also fails, add with empty meeting data
                        enhanced_file = file_data.copy()
                        enhanced_file.update(self._create_empty_meeting_fields())
                        enhanced_files.append(enhanced_file)
    ...
    def _process_date_chunk(self, chunk_start_date: datetime, chunk_files: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            logger.info(
                "AsyncCalendar", "process_chunk", f"Successfully fetched {len(chunk_meetings)} meetings for date range"
            )
            ...
            if chunk_meetings:
                for i, meeting in enumerate(chunk_meetings[:3]):  # Log first 3 meetings
                    logger.debug(
                        "AsyncCalendar", "process_chunk", f"Meeting {i+1}: '{meeting.subject}' at {meeting.start_time}"
                    )
                if len(chunk_meetings) > 3:
                    logger.debug("AsyncCalendar", "process_chunk", f"... and {len(chunk_meetings) - 3} more meetings")
            else:
                logger.warning(
                    "AsyncCalendar",
                    "process_chunk",
    ...
    def _find_best_meeting_match(self, file_datetime: datetime, meetings: List) -> Optional[object]:
        ...
        best_match = None
        best_score = 0
        # Get tolerance from config, same as all other methods
        from config_and_logger import load_config
    ...
    def enhance_files_with_meeting_data_async(
        self, files_dict: List[Dict], callback: Optional[Callable] = None
    ) -> List[Dict]:
        """
        ...
        if SIMPLE_CALENDAR_AVAILABLE and self._calendar_cache_manager:
            work_item = {"type": "sync_files", "files": files_dict, "callback": callback}
            self._calendar_work_queue.put(work_item)

            logger.info(
                "AsyncCalendar", "enhance_files_async", f"Queued {len(files_dict)} files for background calendar sync"
            )
        else:
            logger.debug(
                "AsyncCalendar",
    ...
    def refresh_calendar_data_async(
        self, callback: Optional[Callable] = None, current_files: List[Dict] = None
    ) -> bool:
    ...
    def _process_force_refresh_request(self, work_item: Dict[str, Any]):
        ...
        try:
            ...
            if current_files:
                logger.info(
                    "AsyncCalendar", "force_refresh", f"Repopulating cache for {len(current_files)} current files"
                )
                ...
                repopulate_work_item = {
                    "type": "sync_files",
                    "files": current_files,
                    "callback": lambda files: self._force_refresh_complete_callback(
                        callback, files
                    ),  # Wrap callback to clear flag
                }
                ...
                logger.warning("AsyncCalendar", "force_refresh", "No current files provided for repopulation")
    ...
    def enhance_files_with_meeting_data_sync(self, files_dict: List[Dict]) -> List[Dict]:
        ...
        logger.info(
            "AsyncCalendar", "enhance_sync", f"Synchronously enhancing {len(files_dict)} files with cached meeting data"
        )
        ...
        for file_data in files_dict:
            try:
                ...
                logger.warning(
                    "AsyncCalendar", "enhance_sync", f"Error enhancing {file_data.get('name', 'unknown')}: {e}"
                )
    ...
    def _enhance_single_file_with_cached_data_only(self, file_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if cached_meeting:
                ...
                enhanced_file.update(self._create_empty_meeting_fields())
                logger.debug("AsyncCalendar", "enhance_cached_only", f"No cached meeting data for {filename}")
