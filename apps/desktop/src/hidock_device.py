...
class HiDockJensen:
    ...
    def __init__(self, usb_backend_instance_ref):
        ...
        self._abort_operations = False  # Flag to abort ongoing operations
    ...
    def _perform_health_check(self) -> bool:
        ...
        try:
            try:
                ...
                # Increased timeout to avoid false failures
                device_info = self.get_device_info(timeout_s=5)
                if device_info:
    ...
    def reset_device_state(self):
        """
        Resets the device to a clean state by clearing buffers and resetting USB endpoints.
            ...
            if self.device and self.ep_in and self.ep_out:
                ...
                try:
                    for _ in range(10):  # Limited attempts to avoid infinite loop
                        data = self.device.read(self.ep_in.bEndpointAddress, 64, timeout=50)
                        if not data:
                            break
                        logger.debug("Jensen", "reset_device_state", f"Flushed {len(data)} bytes")
    ...
    def _find_device(self, vid_to_find: int, pid_to_find: int):
        ...
        try:
            ...
            if "no langid" in str(e_val).lower():
                # Device is busy - log only once at debug level
                logger.debug(
                    "Jensen",
                    "_find_device",
                    "Device appears to be in use (no langid available)",
                )
    ...
    def connect(
        ...
        Args:
            ...
            force_reset (bool, optional): Whether to force a device state reset before connecting. Defaults to False.
        ...
        with self._usb_lock:  # Ensure connect/disconnect are serialized
            ...
            # Force reset if requested (useful for recovering from inconsistent states)
            if force_reset:
            ...
            # Clear receive buffer to prevent stale data issues
            self.receive_buffer.clear()
            logger.debug("Jensen", "connect", "Cleared receive buffer before connection")
            ...
            while True:
                ...
                if self._connection_retry_count == 1 and "timeout" in str(error_msg).lower() and not force_reset:
                    logger.info(
                        "Jensen",
                        "connect",
                        "Timeout detected on first retry, attempting device reset",
                    )
                    self.reset_device_state()

                if not auto_retry or not self._should_retry_connection():
                    # Use debug level for device busy errors, error level for others
                    if "access denied" in str(error_msg).lower() or "device busy" in str(error_msg).lower():
                        logger.debug(
                            "Jensen",
                            "connect",
                            f"Device busy after {self._connection_retry_count} attempts",
                        )
                    else:
                        logger.error(
                            "Jensen",
                ...
                # Only log retries at debug level for "device busy" errors
                if "access denied" in str(error_msg).lower() or "in use" in str(error_msg).lower():
                    logger.debug(
                        "Jensen",
                        "connect",
                        f"Device busy, retry attempt {self._connection_retry_count} in {self._retry_delay}s",
                    )
                else:
                    logger.warning(
                        "Jensen",
        ...
        try:
                    ...
                    try:
                        ...
                        if e_detach.errno == 13:  # Permission denied
                            logger.warning(
                                "Jensen",
                                "_attempt_connection",
                                "Permission denied detaching kernel driver. Try running with sudo or check udev rules.",
                            )
            ...
            try:
                ...
                if e_cfg.errno == 16:  # Resource busy
                    logger.debug(  # Log at debug level for busy device
                        "Jensen",
                        "_attempt_connection",
            ...
            try:
                ...
                if e_claim.errno == 16:  # Resource busy (LIBUSB_ERROR_BUSY)
                    logger.debug(  # Log at debug level for busy device
                        "Jensen",
                        "_attempt_connection",
                        "Cannot claim interface - device busy (errno 16)",
                    )
                    self._increment_error_count("connection_lost")
            ...
            # Flush any pending data from the USB endpoints to start fresh
            try:
                # Try to read and discard any pending data (with very short timeout)
                # Do multiple attempts to ensure we get everything
                flush_count = 0
                max_flush_attempts = 10

                while flush_count < max_flush_attempts:
                    try:
                        data = self.ep_in.read(self.ep_in.wMaxPacketSize, timeout=5)
                        if data:
                            flush_count += 1
                            logger.debug(
                                "Jensen", "_attempt_connection", f"Flushed packet {flush_count}: {len(data)} bytes"
                            )
                    except usb.core.USBTimeoutError:
                ...
                if flush_count > 0:
                    logger.info("Jensen", "_attempt_connection", f"Flushed {flush_count} stale packets from USB")

            except Exception as e:
                logger.debug("Jensen", "_attempt_connection", f"Flush error (ignored): {e}")

            # Clear receive buffer one more time after flush
            self.receive_buffer.clear()

            # Reset sequence ID to sync with device
            self.sequence_id = 0
            ...
            if isinstance(e, usb.core.USBError):
                if e.errno == 13:  # Access Denied
                    ...
                    specific_error_message = "Device is currently in use by another application. Please close other apps using the HiDock device and try again."
    ...
    def disconnect(self):
        ...
        """
        # Set abort flag to interrupt any ongoing operations
        self._abort_operations = True

        # Try to acquire lock with timeout to avoid waiting forever
        lock_acquired = self._usb_lock.acquire(timeout=2.0)
        try:
    ...
    def _reset_connection_state(self):
        ...
        self.is_connected_flag = False
        self.receive_buffer.clear()
        self._abort_operations = False  # Reset abort flag
        self.sequence_id = 0  # Reset sequence ID for next connection
        self.device_info = {}
        self.model = "unknown"
        self.device_behavior_settings = {
            "autoRecord": None,
    ...
    def _receive_response(self, expected_seq_id, timeout_ms=5000, streaming_cmd_id=None):
        ...
        while time.time() - start_time < overall_timeout_sec:
            ...
            if self._abort_operations:
                logger.debug("Jensen", "_receive_response", "Operation aborted")
                return None
    ...
    def _calculate_file_duration(self, file_size_bytes, file_version):
        ...
        # Future: use for precise duration calculation
        # _ACTUAL_BITRATE_BPS = 64000  # 64kbps
    ...
    def list_files_cached(self, timeout_s=20, cache_max_age=30):
        ...
        if fingerprint and fingerprint in self._file_list_cache:
            ...
            if self._is_cache_valid(cached_time, cache_max_age):
                logger.info(
                    "Jensen", "list_files_cached", f"Using cached file list ({cached_result['totalFiles']} files)"
                )
        ...
        if result and not result.get("error") and fingerprint:
            ...
            if len(self._file_list_cache) > 5:
                oldest_key = min(self._file_list_cache.keys(), key=lambda k: self._file_list_cache[k][0])
    ...
    async def async_list_files(self, timeout_s=20, use_cache=True, cache_max_age=30):
        ...
        if not hasattr(self, "_usb_executor"):
            self._usb_executor = concurrent.futures.ThreadPoolExecutor(max_workers=2, thread_name_prefix="USB-Async")

        try:
            # Run the blocking USB operation in thread pool
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(self._usb_executor, self._list_files_sync_worker, timeout_s)
            ...
            return {"files": [], "totalFiles": 0, "totalSize": 0, "error": f"Async operation failed: {e}"}
    ...
    def _check_cache_sync(self, cache_max_age):
        ...
        if fingerprint and fingerprint in self._file_list_cache:
            ...
            if self._is_cache_valid(cached_time, cache_max_age):
                logger.info(
                    "Jensen", "async_cache_hit", f"Using cached file list ({cached_result['totalFiles']} files)"
                )
    ...
    def _list_files_sync_worker(self, timeout_s):
        ...
        try:
            ...
            return {"files": [], "totalFiles": 0, "totalSize": 0, "error": f"USB operation failed: {e}"}
    ...
    def list_files_parallel(self, timeout_s=20, min_files_for_parallel=200):
        ...
        if not chunks:
            return {"files": [], "totalFiles": 0, "totalSize": 0, "error": "No data received from device"}
            ...
            with self._usb_lock:
                ...
                while True:
                    response = self._receive_response(
                        seq_id, timeout_ms=adaptive_timeout, streaming_cmd_id=CMD_GET_FILE_LIST
                    )
                    ...
                    if response and response["id"] == CMD_GET_FILE_LIST:
                        ...
                        if consecutive_timeouts >= max_consecutive_timeouts:
                            logger.warning(
                                "Jensen",
                                "parallel_receive",
                                f"Max timeouts reached, collected {len(file_list_chunks)} chunks",
                            )
                            break
                    else:
    ...
    def _parse_file_list_chunks_parallel(self, chunks):
        ...
        try:
            ...
            logger.info(
                "Jensen", "parallel_parse", f"Processing {total_files_from_header} files with optimized serial parsing"
            )
    ...
    async def async_list_files_parallel(self, timeout_s=20, min_files_for_parallel=200):
        ...
        if not hasattr(self, "_parallel_executor"):
            self._parallel_executor = concurrent.futures.ThreadPoolExecutor(
                max_workers=1, thread_name_prefix="Parallel-USB"
            )

        try:
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(
                self._parallel_executor, self.list_files_parallel, timeout_s, min_files_for_parallel
            )
            ...
            return {"files": [], "totalFiles": 0, "totalSize": 0, "error": f"Async parallel operation failed: {e}"}

    def list_files_with_retry(self, timeout_s=20, max_retries=2):
        """
        List files with automatic retry for incomplete transfers.

        Args:
            timeout_s (int): Timeout for each attempt
            max_retries (int): Maximum number of retries for incomplete data

        Returns:
            dict: File list result with retry information
        """
        for attempt in range(max_retries + 1):
            # Check if we should abort
            if self._abort_operations:
                logger.info("Jensen", "list_files_with_retry", "Operation aborted")
                return {"files": [], "totalFiles": 0, "totalSize": 0, "error": "Operation aborted"}

            logger.info("Jensen", "list_files_with_retry", f"Attempt {attempt + 1}/{max_retries + 1} to get file list")

            result = self.list_files(timeout_s)

            # If successful and complete, return immediately
            if result and not result.get("incomplete"):
                if attempt > 0:
                    logger.info("Jensen", "list_files_with_retry", f"Success on attempt {attempt + 1}")
                return result

            # If incomplete, decide whether to retry
            if result and result.get("incomplete"):
                files_received = len(result.get("files", []))
                expected_count = result.get("expected", 0)

                logger.warning(
                    "Jensen",
                    "list_files_with_retry",
                    f"Attempt {attempt + 1} incomplete: {files_received}/{expected_count} files",
                )

                # Don't retry if we got a decent amount of data and it's the last attempt
                if attempt == max_retries:
                    logger.error(
                        "Jensen",
                        "list_files_with_retry",
                        f"Final attempt failed, returning partial data: {files_received}/{expected_count}",
                    )
                ...
                retry_delay = 1.0 * (attempt + 1)  # Increasing delay
                logger.info("Jensen", "list_files_with_retry", f"Waiting {retry_delay}s before retry...")
                time.sleep(retry_delay)

            else:
                # Complete failure, retry with longer timeout
                logger.error("Jensen", "list_files_with_retry", f"Attempt {attempt + 1} failed completely")
                if attempt < max_retries:
                    timeout_s = min(timeout_s * 1.5, 60)  # Increase timeout for retry

        # All retries failed
        logger.error("Jensen", "list_files_with_retry", f"All {max_retries + 1} attempts failed")
        return {
            "files": [],
            "totalFiles": 0,
            "totalSize": 0,
            ...
            with self._usb_lock:
                ...
                def file_list_handler(response_data):
                    ...
                    if not response_data or len(response_data) == 0:
                        ...
                        logger.info(
                            "Jensen",
                            "list_files",
                            f"Empty response received, completing file list with {len(file_list_chunks)} chunks",
                        )
                    ...
                    if expected_file_count is not None:
                        # Conservative heuristic: ~60 bytes per file (actual data shows 57 bytes/file)
                ...
                while final_files is None:
                    ...
                    if response and response["id"] == CMD_GET_FILE_LIST:
                        ...
                        if consecutive_timeouts >= max_consecutive_timeouts:
                            # Check if we have a reasonable amount of data before giving up
                            if file_list_chunks and total_bytes_received > 100:
                                logger.info(
                                    "Jensen",
                                    "list_files",
                                    f"Timeout after {len(file_list_chunks)} chunks ({total_bytes_received} bytes), processing available data",
                                )
                            else:
                                logger.warning(
                                    "Jensen",
                                    "list_files",
                                    f"Max timeouts reached with minimal data ({len(file_list_chunks)} chunks, {total_bytes_received} bytes)",
                                )
                ...
                if expected_file_count and len(final_files) < expected_file_count:
                    logger.error(
                        "Jensen",
                        "list_files",
                        f"INCOMPLETE DATA: Received only {len(final_files)}/{expected_file_count} files! "
                        f"Missing {expected_file_count - len(final_files)} files. "
                        f"This may indicate a USB communication problem or device issue.",
                    )
                    ...
                    return {
                        ...
                        "missing": expected_file_count - len(final_files),
                        "error": f"Incomplete file list: {len(final_files)}/{expected_file_count} files received",
                    }
    ...
    def _parse_file_list_chunks(self, chunks):
        ...
        if len(data) >= 6 and data[parse_offset] == 0xFF and data[parse_offset + 1] == 0xFF:
            total_files_from_header = self._binary_struct_formats["header_count"].unpack_from(data, parse_offset + 2)[0]
            parse_offset += 6
        ...
        while parse_offset < len(data) and (
            total_files_from_header == -1 or parsed_file_count < total_files_from_header
        ):
            try:
