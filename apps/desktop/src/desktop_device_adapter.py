...
class DesktopDeviceAdapter(IDeviceInterface):
    ...
    async def connect(
        ...
        Args:
            ...
            force_reset: Whether to force a device state reset before connecting
        ...
        try:
            ...
            if "access denied" in error_str or "device busy" in error_str or "in use" in error_str:
                logger.debug("DesktopDeviceAdapter", "connect", f"Device busy: {e}")
            else:
    ...
    async def get_recordings(self) -> List[AudioRecording]:
        ...
        try:
            # Use retry mechanism to handle incomplete transfers more robustly
            files_info = self.jensen_device.list_files_with_retry(timeout_s=20, max_retries=2)
            ...
            if "error" in files_info:
                ...
                if files_info.get("incomplete") and files_info.get("files"):
                    ...
                    # For other errors, raise exception to trigger retry/fallback
                    raise ConnectionError(f"Failed to get complete file list: {error_msg}")
            ...
            # Check if this is an expected abort
            error_str = str(e).lower()
            if "aborted" in error_str or "operation aborted" in error_str:
                logger.debug(
                    "DesktopDeviceAdapter",
                    "get_recordings",
                    f"File list operation cancelled: {e}",
                )
            else:
                logger.error(
                    "DesktopDeviceAdapter",
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            device_info = self.jensen_device.get_device_info(timeout_s=2)
    ...
    def reset_device_state(self):
        """Reset the device to a clean state to recover from communication errors."""
        try:
            self.jensen_device.reset_device_state()
            logger.info("DesktopDeviceAdapter", "reset_device_state", "Device state reset successful")
        except Exception as e:
            logger.error("DesktopDeviceAdapter", "reset_device_state", f"Device reset failed: {e}")
