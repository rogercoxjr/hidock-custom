...
from outlook_calendar_service import OUTLOOK_AVAILABLE, MeetingMetadata, OutlookCalendarService
...
class TestOutlookCalendarService:
    ...
    def test_authenticate_success(self, mock_account_class, outlook_service):
        ...
        mock_account_class.assert_called_once_with(
            ("test_client_id", "test_client_secret"), auth_flow_type="authorization", tenant_id="common"
        )

        # Verify authentication was attempted with correct scopes
        mock_account.authenticate.assert_called_once_with(scopes=["https://graph.microsoft.com/Calendars.Read"])
