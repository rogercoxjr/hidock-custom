...
- **Known flaky test (pre-existing):** `electron/server/__tests__/transcripts.test.ts` › "POST /api/recordings/:id/transcription/retry … sets recording status to pending" is **non-deterministic**. The route correctly writes `pending` synchronously, then fire-and-forgets `processQueueManually()`; in the test env (no ASR/LLM key) that async run fails and flips status to `error` before the assertion — a race, not a product bug. A naive file-level `vi.mock` of the transcription module makes it deterministic **but breaks** the sibling `queue/process` / `retry-failed` / `processor/start` tests that legitimately drive the real processor (they 500 when `startTranscriptionProcessor`/processing are stubbed away). **Correct fix (deferred):** make *only this test* deterministic — e.g. inject a per-test spy that no-ops `processQueueManually` for the retry case only (module-scoped `vi.mock` with `vi.mocked(...).mockImplementationOnce` reset per test), or split the retry test to assert the synchronous `pending` write via a direct DB read taken inside the route's transaction boundary. Do NOT blanket-mock the module. Until fixed, this is the **1 expected red** in an otherwise-green suite (`vitest run` → 1 failed / ~3580 passed).
